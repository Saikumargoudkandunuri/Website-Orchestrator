...
class Provider(BaseProvider):
	...
	async def chat(self, request: ChatRequest) -> ChatResponse:
		return ChatResponse(text="[huggingface stub] chat not implemented", tokens=TokenUsage())

	async def generate(self, request: GenerateRequest) -> GenerateResponse:
		return GenerateResponse(text="[huggingface stub] generate not implemented", tokens=TokenUsage())
	...
	async def models(self) -> list[ModelInfo]:
		return [ModelInfo(provider="huggingface", model="hf-model")]
