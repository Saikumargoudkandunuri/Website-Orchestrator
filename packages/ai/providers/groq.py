...
class Provider(BaseProvider):
	...
	async def chat(self, request: ChatRequest) -> ChatResponse:
		return ChatResponse(text="[groq stub] chat not implemented", tokens=TokenUsage())

	async def generate(self, request: GenerateRequest) -> GenerateResponse:
		return GenerateResponse(text="[groq stub] generate not implemented", tokens=TokenUsage())
	...
	async def models(self) -> list[ModelInfo]:
		return [ModelInfo(provider="groq", model="llama-3-8b")]
