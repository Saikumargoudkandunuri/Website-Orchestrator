...
class Provider(BaseProvider):
	...
	async def chat(self, request: ChatRequest) -> ChatResponse:
		return ChatResponse(text="[anthropic stub] chat not implemented", tokens=TokenUsage())

	async def generate(self, request: GenerateRequest) -> GenerateResponse:
		return GenerateResponse(text="[anthropic stub] generate not implemented", tokens=TokenUsage())
	...
	async def models(self) -> list[ModelInfo]:
		return [ModelInfo(provider="anthropic", model="claude-3-5-sonnet")]
