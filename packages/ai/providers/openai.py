...
class Provider(BaseProvider):
	...
	async def chat(self, request: ChatRequest) -> ChatResponse:
		return ChatResponse(text="[openai stub] chat not implemented", tokens=TokenUsage())

	async def generate(self, request: GenerateRequest) -> GenerateResponse:
		return GenerateResponse(text="[openai stub] generate not implemented", tokens=TokenUsage())
	...
	async def models(self) -> list[ModelInfo]:
		return [ModelInfo(provider="openai", model="gpt-4o")]
