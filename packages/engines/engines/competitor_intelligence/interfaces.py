"""Competitor Intelligence Engine interface (§4.5)."""
...
__all__ = ["CompetitorIntelligenceEngine"]


class CompetitorIntelligenceEngine:
    """Sitewide Competitor Intelligence Engine (§4.5)."""
    ...
    def analyze(
        ...
        if not isinstance(request.target, SiteTarget):
            return Err(EngineAnalysisError("CompetitorIntelligenceEngine requires a SiteTarget"))
        start = time.perf_counter()
        try:
            from engines.competitor_intelligence.services import CompetitorIntelligenceService  # noqa: PLC0415
            service = CompetitorIntelligenceService()
            report: CompetitorIntelligenceReport = service.analyze(
                request.target.site_id,
                site_context=request.site_context,
                options=request.options,
            )
        except Exception as exc:
            return Err(EngineAnalysisError(f"CompetitorIntelligenceEngine failed: {exc}"))
